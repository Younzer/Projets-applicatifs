...
class Solver():
    ...
    def getSolution(self):
        '''Return an array with values of each cases of the final solution of the sudoku'''   
        return [x for x in self.finalModel if x > 0]

    def getRandomHint(self):
        '''Return a random hint'''        
        return random.choice(self._hints)    
...
'''
    ...
    def removeContradictions(hints):
        '''This function manage with potential contradictions generated by generateFixedHints(),
        for example when hints = [218, 217, 396, 394], it will return [218, 396]'''
        hints = np.array(hints) 
        restes = hints % 10
        coords = hints - restes
        for coord in np.unique(coords): 
            indices = np.where(coords == coord)
            if len(indices[0]) > 1:  # If more of one hint in same case, we keep only one 
                hints = np.delete(hints, indices[0][1:])
        return hints 
    ...
    def generateRandomGrid():
        ...
        while not satisfiable:   
            ...
            for hint in fixedHints:
                solver.addClause([hint])
    ...
    if len(sys.argv) > 1:
        solver, result = generateRandomGrid()
    ...
    solver._hints = solver.getSolution()
    ...
    while not anotherSolution: # We get rid of hints until we find another solution
        ...
        newSolver.buildDataStructure()        
        resultat = newSolver.solve()
        if resultat == newSolver._cst.lit_True:            
